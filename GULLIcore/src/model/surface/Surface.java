...
//    public final HashMap<Integer, SurfaceTriangle> triangleCapacity;
    public final SurfaceTriangle[] triangleCapacitys;
...
//    private final HashMap<Integer, float[]> actualVelocity = new HashMap<>(0);
    ...
    /**
     * Stores Measurements for Triangles.
     */
//    private HashMap<Integer, TriangleMeasurement> measurements = new HashMap<>(1);
//    /**
//     * Should be the same list as in the network.
//     */
...
//                        
//                        //TODO Request for later use
//                        if (neighbourvelocity[id] == null) {
//                            //Only request, if no triangleVelocity is loaded.
//                            if(triangleVelocity==null||triangleVelocity[id]==null)
//                            try {
//                                initVelocityToNeighbours(id);
//                            } catch (Exception e) {
//                                e.printStackTrace();
//                            }
//                        }
...
//                        tri.measurement = measurements.get(id);
//                    } else {
//                        //Create measurement
//                        if (times != null) {
//                            createMeasurement(id, times);
//                            tri.measurement = measurements.get(id);
//                        }
//                    }
//                    /**
//                     * Set Measurement from object into the surface's raster for
//                     * faster access in reports.
//                     */
//                    measurementRaster[id] = tri.measurement;
                }
            }
        }
        return tri;
...
//        ny /= lengthNN;
        ...
        if (true/*cos < 0.6f&&cos>-0.6*/) {
            ...
            //Test for unphysical solution
//        if (Math.abs(lengthVN) > 1) {
//            lengthVN = Math.signum(lengthVN) * 1;
//        }
        }
...
//    public HashMap<Integer, SurfaceTriangle> getTriangles() {
//        int counter = 0;
//        for (SurfaceTriangle triangleCapacity : triangleCapacitys) {
//            if (triangleCapacity != null) {
//                counter++;
//            }
//        }
//        HashMap<Integer, SurfaceTriangle> map = new HashMap<>(counter);
//        for (int i = 0; i < triangleCapacitys.length; i++) {
//            if (triangleCapacitys[i] != null) {
//                map.put(i, triangleCapacitys[i]);
//            }
//        }
//        return map;
//    }
     ...
     */
    public void reset() {
//        System.out.println(getClass() + "::reset() resetTriangles.particlecount");
        nextRecalculation = 0L;
        this.timeIndex = 0;
        this.statistics.clear();
        clearTriangleCapacities();
        if (measurementRaster != null) {
            measurementRaster.reset();
        }
//        deleteMeasuredValues();
        sourcesForSpilloutParticles.clear();
    }

//    public void deleteMeasuredValues() {
////        for (TriangleMeasurement tm : measurements.values()) {
////            tm.reset(numberOfMaterials);
////        }
//    }
//    public void clearMeasurementPositions() {
////        measurements.clear();
//    }
    /**
...
//        this.deleteMeasuredValues();
    ...
    public void clearTriangleCapacities() {
        for (int i = 0; i < triangleCapacitys.length; i++) {
            SurfaceTriangle tc = triangleCapacitys[i];
            if (tc == null) {
                continue;
            }
...
//                if (tc.measurement != null) {
//                    tc.measurement.reset(numberOfMaterials);
//                }
            } else {
                //clear completely
                triangleCapacitys[i] = null;
            }
        }
    }
    ...
    public void applyStreetInlets(Network nw, Collection<Pair<String, Integer>> pipeANDTriangleIDs) {
        ...
        for (Pair<String, Integer> pipeANDTriangleID : pipeANDTriangleIDs) {
            ...
            if (tri == null) {
                System.err.println("Could not find a Traingle with ID '" + triangleID + "' to create an inlet next to pipe " + pipename);
                continue;
            }
...
//                    inlets.put(triangleID, inlet);
//                }else{
//                    System.out.println("Inlet is connected to " + cap + ". Connection for this type of capaciy not yet implemented.");
//                }
//                inlets[triangleID] = inlet;
            } catch (Exception e) {
                e.printStackTrace();
...
//        }
    ...
    public void addStatistic(Particle p, SurfaceTriangle start, Inlet inlet, Manhole manhole, long traveltime) {
        ...
        if (stat == null && manhole != null) {
            synchronized (statistics) {
                for (SurfacePathStatistics st : statistics) {
                    if (st.pathEquals(start, manhole)) {
                        stat = st;
                        break;
                    }
                }
        ...
        if (stat == null) {
            ...
            if (inlet != null) {
                stat = new SurfacePathStatistics(start, inlet);
            } else if (manhole != null) {
                stat = new SurfacePathStatistics(start, manhole);
            }
            if (stat != null) {
                synchronized (statistics) {
                    statistics.add(stat);
...
//        }
        ...
        double paltx = p.getPosition2d_actual().x;
        double palty = p.getPosition2d_actual().y;
...
//            System.out.println("v1:"+ velocityNodes[triangleNodes[triangleID][0]][(int) timeIndex][0]+"  tri:"+triangleNodes[triangleID][0]+" time: "+timeIndex+"   w0="+w[0]);
            ...
            //If no weights are calculated, yust use mean velocity from neighbouring triangle nodes.
            if (timeInterpolatedValues) {
...
//        float[] pos = new float[]{(float) x, (float) y};
        ...
        if (outs == 0) {
            ...
            int[][] edgeIndices = new int[outs][2];
...
//            System.out.println("Check Edge "+edgeIndices[0]+" - "+edgeIndices[1]);
...
//            if (neighbourID < 0 || leftIterations < 1) 
            {
...
//                        System.out.println("PARALLEL");
                ...
                throw new BoundHitException(id, new double[]{xneu, yneu});
            }
        }
//        throw new BoundHitException(id, new double[]{triangleMids[id][0], triangleMids[id][1]});
    }
    ...
    public int getTargetTriangleID_RIss(Particle p, int id, double xold, double yold, double x, double y) throws BoundHitException {
        ...
        for (int f = 0; f < 25; f++) {
            ...
            if (closeNodeOld == closeNode) {
                double[] palt = {xold, yold};
                throw new BoundHitException(id, palt);
            }
            zähler += 1;
        }
        // warum hat er immer noch nicht das triangle gefunden?
        //System.out.println("after " + zähler + " loops -> still no triangle found. particle id: " + p.getId() + ", position: " + p.getPosition2d_actual() + ", old triID: " + id + ", new triID: " + triangleHistory[0][0]);
                    ...
                    if (Math.abs((y - yold) / (x - xold) - (c[1] - a[1]) / (c[0] - a[0])) > 0.0000001) {
                        ...
                        if (intersec[0] >= 0) {
                            ...
                            if (intersec[0] >= 0) {
                                ...
                                throw new BoundHitException(triangleID, intersec);
                            }
                        }
                    } else {
                        //Lines are parallel
...
//        System.err.println("Surface.setTimeContainer: "+times);
    ...
    public class BoundHitException extends Exception {
        ...
        public final int id;
        public final double[] correctedPosition;

        public BoundHitException(int id, double[] correctedPosition) {
            this.id = id;
