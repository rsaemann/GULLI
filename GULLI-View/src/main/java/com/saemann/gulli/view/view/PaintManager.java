...
//            System.out.println(getClass() + "::addPipesPaint -> return because null pointer");
        ...
        try {
            ...
            for (final Pipe pipe : network.getPipes()) {
                ...
                ArrayList<model.GeoPosition2D> list = toRGIS(listG);
...
//                                    return super.paint(g2);
                    ...
                    LabelPainting lp = new LabelPainting(i, MapViewer.COLORHOLDER_LABEL, new model.GeoPosition(longlat), 20, -5, -5, df3.format(lvl)) {
                ...
                for (int i = 0; i < surface.getTriangleNodes().length; i++) {
                    for (int j = 0; j < 3; j++) {
                        ...
                        try {
                            ...
                            Coordinate midPoint = surface.getGeotools().toGlobal(new Coordinate(mid[0], mid[1]));
                            LabelPainting av = new LabelPainting(id, chSpillover, new model.GeoPosition(midPoint), new String[]{mid[2] + ""});
...
//    }
    ...
    public void selectLocationID(Object o, String string, long id) {
        ...
        selectedLayer = null;
        model.GeoPosition mousePoint;
        if (mapViewer.clickPoint != null) {
            mousePoint = new model.GeoPosition(mapViewer.clickPoint.x, mapViewer.clickPoint.y);
        } else {
            mousePoint = new model.GeoPosition(mapViewer.getCenter().x, mapViewer.getCenter().y);
        }
        try {
            if (string == null) {
                return;
...
//                            position = mh.getPosition3D(0);
                ...
                LabelPainting lp = new LabelPainting(0, MapViewer.COLORHOLDER_LABEL, new model.GeoPosition(mapViewer.clickPoint.x, mapViewer.clickPoint.y), text);
                ...
                for (InjectionInformation injection : injections) {
                    if (injection.getId() % injections.size() == id) {
                        ...
                        LabelPainting lp = new LabelPainting(0, MapViewer.COLORHOLDER_LABEL, new model.GeoPosition(injection.getPosition().getLatitude(), injection.getPosition().getLongitude()), str.toString().split(";"));
                        mapViewer.addPaintInfoToLayer(mapViewer.LAYER_KEY_LABEL, lp);
                        selectedLayer = string;
                        selectedID = id;
                        mapViewer.recalculateShapes();
...
//                    PaintInfo pi = new ShapePainting(pos, ShapeEditor.SHAPES.CROSS.getShape(), 0, 0, id++, chInjectionLocation, chInjectionLocation.getStroke());
                    NodePainting np = new NodePainting(in.getId(), new model.GeoPosition(pos.getLatitude(), pos.getLongitude()), chInjectionLocation);
//                    {
//                        @Override
//                        public boolean paint(Graphics2D g2) {
...
//            System.out.println("is painted? "+pant);
    ...
    public static ArrayList<model.GeoPosition2D> toRGIS(ArrayList<com.saemann.gulli.core.model.GeoPosition2D> list) {
        ArrayList<model.GeoPosition2D> newlist = new ArrayList<>(list.size());
        for (com.saemann.gulli.core.model.GeoPosition2D geoPosition : list) {
            model.GeoPosition gp = new model.GeoPosition(geoPosition.getLatitude(), geoPosition.getLongitude());
            newlist.add(gp);
        }
        return newlist;
    }
