...
public class PaintManager implements LocationIDListener, LoadingActionListener, SimulationActionListener, ParticleListener {
    ...
    public static final String layerInlets = "INLET";
    ...
    private final ColorHolder chInjectionLocation = new ColorHolder(Color.RED, "Injection");
    ...
    private final DoubleColorHolder chTrianglesGrid = new DoubleColorHolder(Color.orange, new Color(1, 1, 1, 0), "Surface Ttriangles");
...
//    ColorHolder chTriangleMids = new ColorHolder(Color.orange, "Triangle Mids");
    ...
    private Point2D.Double posTopLeft, posBotRight;
...
//    private Scenario scenario;
    ...
    public PaintManager(Controller con, SimpleMapViewerFrame frame) {
        ...
        try {
            geoTools = new GeoTools("EPSG:4326", "EPSG:25832", StartParameters.JTS_WGS84_LONGITUDE_FIRST);
...
//                if (mh.tags != null && mh.tags.containsKey("Water")) 
                {
                    try {
                        int w = 0;//Integer.parseInt(mh.tags.get("Water"));
...
//        mapViewer.zoomToFitLayer();
    ...
    public void addInletsPaint(Surface surface, boolean drawConnectionToPipe) {
        //Inlets
        ...
        if (surface.getInlets() != null) {
//                System.out.println(getClass()+" searching "+surface.getTriangles().values().size()+" triangles for inlets.");
            ColorHolder chinlet = new ColorHolder(Color.cyan, "Inlet");
            ColorHolder chinletPipe = new ColorHolder(Color.cyan, "Inlet's pipe");
            int counter = 0;
            for (int i = 0; i < surface.getInlets().length; i++) {
                Inlet inlet = surface.getInlets()[i];
...
//                        System.out.println("Add Inlet at Traingle "+tri.getManualID());
                    if (drawConnectionToPipe && in.getNetworkCapacity() != null) {
                        ...
                        Coordinate c0 = inlet.getPosition().latLonCoordinate();
                        Coordinate c1 = target.latLonCoordinate();
...
//                System.out.println(pipe.getFlowInletConnection().getPosition());     
//                System.out.println(pipe.getFlowOutletConnection().getPosition());
...
//                    ap = new ArrowPainting(pipe.getAutoID(), list, chPipes);
//                    mapViewer.addPaintInfoToLayer(layerPipes, ap);
//                    if (!drawPipesAsArrows) {
//                        ap.arrowheadvisibleFromZoom = 200;
//                    }
...
//                                float sum = 0;
//                                for (int i = 0; i <= pipe.getMeasurementTimeLine().getContainer().getActualTimeIndex(); i++) {
//                                    sum += pipe.getMeasurementTimeLine().getMass(i);
//                                    if (sum > 0.001) {
//                                        g2.setColor(ch.color);
...
//                    }
                    ...
                    ArrowPainting ap = new ArrowPainting(pipe.getAutoID(), list, chPipes) {
                        @Override
                        public boolean paint(Graphics2D g2) {
//                        int index = ArrayTimeLinePipeContainer.instance.getActualTimeIndex();
                            double refV = 1;
                            if (pipe.getStatusTimeLine() instanceof ArrayTimeLinePipe) {
                                refV = ((ArrayTimeLinePipe) pipe.getStatusTimeLine()).getV_max();
                            }
                            double v = pipe.getStatusTimeLine().getVelocity();//ArrayTimeLineMeasurement.getActualTimeIndex());
                            if (v >= 0) {
                                Color co = getColorHolderVelocityRelative((int) (v * 100. / refV)).color;
                                g2.setColor(co);
//                            System.out.println(getClass() + ":: color velocity:" + co);
                            } else {
                                Color co = getColorHolderVelocityRelative((int) (-v * 100. / refV)).color;
                                g2.setColor(co);
                            }
                    ...
                    ArrowPainting ap = new ArrowPainting(pipe.getAutoID(), list, chPipes) {
                        @Override
                        public boolean paint(Graphics2D g2) {
                            double h = pipe.getStatusTimeLine().getWaterlevel();//ArrayTimeLineMeasurement.getActualTimeIndex());
                            double refh = h;
...
//                    ColorHolder chloaded=new ColorHolder(Color.green, "SparseTimeline Loaded");
//                    ColorHolder chnotSet=new ColorHolder(Color.red, "SparseTimeline Empty");
//                    
                    String layer = layerPipes;
...
//                    String layer = layerSurfaceContaminated;
//                    Color color = Color.magenta;
                    int counter = 0;
                ...
                if (dataloader != null) {
                    if (dataloader instanceof HE_Database) {
                        ...
                        for (String name : names) {
                            ...
                            if (mh != null) {
                                NodePainting np = new NodePainting(mh.getAutoID(), mh.getPosition().latLonCoordinate(), chSpillover);
...
//                                    NodePainting np = new NodePainting(i, tri.getPosition3D(0).latLonCoordinate(), new ColorHolder(color));
                                    np.setRadius(2);
                                    mapViewer.addPaintInfoToLayer(layerSurfaceContaminated, np);
            ...
            try {
                ...
                if (surface.getTriangleVelocity() != null) {
                    for (int i = 0; i < surface.triangleNodes.length; i++) {
                        ...
                        Coordinate vTtarget = surface.getGeotools().toGlobal(new Coordinate(mid[0] + velocityFactor * v[0], mid[1] + velocityFactor * v[1]));
                ...
                if (surface.isNeighbourVelocityLoaded()) {
                    for (int i = 0; i < surface.triangleNodes.length; i++) {
                        for (int n = 0; n < 3; n++) {
                            ...
                            float v = 0;
                            try {
                                v = surface.getVelocityToNeighbour(t, i, n);
                            } catch (Exception e) {
...
//                mapViewer.addLineStringsColored(layerPipes, pipe.getAutoID(), list, chPipes, chPipes.getStroke());
    ...
    public void addParticle(final Particle p) {
        ...
        if (p.getPosition3d() == null || (Math.abs(p.getPosition3d().getLongitude()) < 0.01 && Math.abs(p.getPosition3d().getLatitude()) < 0.01)) {
        ...
        if (p.getClass().equals(HistoryParticle.class)) {
            ...
            NodePainting np = new NodePainting(p.getId(), hp, chParticles) {
                @Override
                public boolean paint(Graphics2D g2) {
                    if (p.isInactive()) {
                        return false;
...
//                    g2.setColor(Color.cyan);
        ...
        NodePainting np = new NodePainting(p.getId(), p, chParticles) {
            @Override
            public boolean paint(Graphics2D g2) {
                if (p.isInactive()) {
                    return false;
...
//            count++;
//            if (count % 100000 == 0) {
////                System.out.println("added " + count + " particle shapes to paintmanager.");
//            }
        }
        //Save space from the cleared list
        System.gc();
//        System.out.println("added all shapes for particles. ");
    }
...
//        System.out.println("rel:"+relConcentration+"\t index="+index);
    ...
    public void selectLocationID(Object o, String string, long l) {
        selectedLayer = null;
        GeoPosition2D mousePoint = new GeoPosition(mapViewer.clickPoint.x, mapViewer.clickPoint.y);
        try {
            if (string == null) {
                return;
            }
            if (string.equals(layerParticle)) {
                for (NodePainting particlePainting : particlePaintings) {
                    if (particlePainting.getId() == l) {
                        String key = mapViewer.LAYER_KEY_LABEL;
                        Particle p = (Particle) particlePainting.getPosition();
                        String information = "Particle " + p.getId() + ";V = " + df3.format(p.getVelocity1d()) + " m/s;Pos " + df3.format(p.getPosition1d_actual()) + "m;Haltung:" + p.getSurrounding_actual() + ";Travel:" + (int) p.getTravelledPathLength() + "m";
                        if (p.getSurrounding_actual() instanceof Pipe) {
                            Pipe pipe = (Pipe) p.getSurrounding_actual();
                            information = information + ";relPos " + df.format(p.getPosition1d_actual() / pipe.getLength()) + "";
                            ...
                            if (st.manhole != null) {
                                information += ";>Manhole " + st.manhole;
                            }
                            if (st.inlet != null) {
...
//                        System.out.println("clicked Particle: Pos = " + p.getPosition1d_actual() + "\tV=" + p.getVelocity1d() + "m/s");
                        ...
                        LabelPainting lp = new LabelPainting(0, MapViewer.COLORHOLDER_LABEL, p, text);
...
//                System.out.println("showPipeText " + l);
                ...
                for (Pipe p : network.getPipes()) {
                    if (p.getAutoID() == l) {
                        ...
                        //Position
//                        System.out.println("showPipeText  drain");
...
//                System.out.println("clicked on triangle");
                ...
                if (surf != null&&l>=0) {
                    ...
                    if (surf.getWaterlevels() != null && surf.getWaterlevels().length > 0) {
                        wls = surf.getWaterlevels()[(int) l];
                    } else {
                        wls = surf.loadWaterlevels((int) l);
                    }
                    ...
                    if (surf.getMeasurementRaster() != null && surf.getMeasurementRaster() instanceof SurfaceMeasurementTriangleRaster) {
                        ...
                        TriangleMeasurement measurement = raster.getMeasurements()[(int) l];
...
//        if (mh.tags != null && !mh.tags.isEmpty()) {
//            for (Map.Entry<String, String> tag : mh.tags.entrySet()) {
...
//            str += "--- Tags ---;";
//            int i = 0;
//            for (Map.Entry<String, String> tag : p.tags.entrySet()) {
//                str += tag.getKey() + " " + tag.getValue() + ";";
//                i++;
//            }
//
    ...
    public void setSelectedPipe(long id) {
        for (Pipe pipe : network.getPipes()) {
            if (pipe.getAutoID() == id) {
                ...
                list.add(pipe.getFlowInletConnection().getPosition());
                list.add(pipe.getFlowOutletConnection().getPosition());
//                mapViewer.addLineStringsColored(mapViewer.l, pipe.getAutoID(), list, chPipes, chPipes.getStroke());
...
//        for (Pipe pipe : network.getPipesSewer()) {
//            if (pipe.getAutoID() == autoID) {
//                ArrayList<GeoPosition2D> list = new ArrayList<>(2);
//                list.set(pipe.getFlowInletConnection().getPosition());
//                list.set(pipe.getFlowOutletConnection().getPosition());
////                mapViewer.addLineStringsColored(mapViewer.l, pipe.getAutoID(), list, chPipes, chPipes.getStroke());
//                ColorHolder chTriangleMids = new ColorHolder(Color.red, "Selected");
//                LinePainting lp = new LinePainting(0, list, chTriangleMids, new BasicStroke(2));
//                mapViewer.addPaintInfoToLayer("SELECT", lp);
//                mapViewer.setSelectedObject(lp);
//                mapViewer.recalculateShapes();
//                mapViewer.repaint();
...
//        }
    ...
    public void showPipeFillRate(int timeIndex) {
        ...
        mapViewer.clearLayer(layerPipes);
//        System.out.println("showPipefillrate: velocities.length="+chVelocity.length);
...
//                System.out.println("f="+f+"-> use index: "+(int) (f));
...
//        mapViewer.repaint();
    ...
    public void updateUTMReference() {
        ...
        if (posTopLeft == null) {
            posBotRight = new Point2D.Double();
            posTopLeft = new Point2D.Double();
        }
        try {
            Coordinate topleft = geoTools.toUTM(new Coordinate(tl.y, tl.x));
            Coordinate bottright = geoTools.toUTM(new Coordinate(br.y, br.x));
...
//            double distance = 0;
            for (int i = 0; i < triangleMids.length; i++) {
                try {
                    double posX = 0;// triangleMids[i][0];
                    double posY = 0;// triangleMids[i][1];
                    ...
                    double pox = triangleMids[i][0];
                    double poy = triangleMids[i][1];

//                    distance += Math.sqrt((posX - pox) * (posX - pox) + (posY - poy) * (posY - poy));
//                    Coordinate p = geoTools.toGlobal(new Coordinate(posX, posY));
                    Coordinate p = SHP_IO_GULLI.transform(new Coordinate(posX, posY), transformUTM_WGS);
                    NodePainting np = new NodePainting(i, p, ch);
//                    mapViewer.addPaintInfoToLayer(layerPipes, np);

//                    mapViewer.addPaintInfoToLayer("MID", new NodePainting(0, SHP_IO_GULLI.transform(new Coordinate(pox, poy), transformUTM_WGS), new ColorHolder(Color.white)));
//                    mapViewer.addPaintInfoToLayer("NODE", new NodePainting(0, SHP_IO_GULLI.transform(new Coordinate(vertices[triangleindizes[i][0]][0], vertices[triangleindizes[i][0]][1]), transformUTM_WGS), chTriangleMids));
//                    mapViewer.addPaintInfoToLayer("NODE", new NodePainting(0, SHP_IO_GULLI.transform(new Coordinate(vertices[triangleindizes[i][1]][0], vertices[triangleindizes[i][1]][1]), transformUTM_WGS), chTriangleMids));
//                    mapViewer.addPaintInfoToLayer("NODE", new NodePainting(0, SHP_IO_GULLI.transform(new Coordinate(vertices[triangleindizes[i][2]][0], vertices[triangleindizes[i][2]][1]), transformUTM_WGS), chTriangleMids));
//                    mapViewer.addPaintInfoToLayer("CalcMid", new NodePainting(0, SHP_IO_GULLI.transform(new Coordinate(posX, posY), transformUTM_WGS), new ColorHolder(Color.yellow)));
//                    System.out.println("Calculated Mid: " + posX + ", " + posY + "   read Mid= " + pox + "," + poy);
//
//                    if (true) {
//                        if (i > 2) {
//                            break;
...
//                    System.out.println("sx="+sx+",  sy="+sy+"\t l="+l);
                        if (Math.abs(l) < 0.0001) {
//                        System.out.println("   NOT affected");
                            continue;
                        }
//                    System.out.println("sx*v/l="+(sx*v/l));
                        vx += sx * v / l;
                        vy += sy * v / l;
                        outs++;
                    }
...
//            System.out.println("Summe distance= " + distance + " = " + (distance / (double) triangleMids.length) + "m/Dreieck");
//            mapViewer.zoomToFitLayer();
//            mapViewer.repaint();
...
//            DoubleColorHolder dch = new DoubleColorHolder(Color.orange, new Color(200, 200, 200, 100), layerSurfaceBoundary);
//            AreaPainting ap = new AreaPainting(0, dch, surface.calcConvexHullWGS84());
//            ap.recomputeShape(mapViewer);
//            mapViewer.addPaintInfoToLayer(layerSurfaceBoundary, ap);
            try {
                addInletsPaint(surface, false);
            } catch (Exception e) {
            }
//
            mapViewer.recalculateShapes();
...
//                    PaintInfo pi = new ShapePainting(pos, ShapeEditor.SHAPES.CROSS.getShape(), 0, 0, id++, chInjectionLocation, chInjectionLocation.getStroke());
                    NodePainting np = new NodePainting(id++, pos, chInjectionLocation);
//                    {
//                        @Override
//                        public boolean paint(Graphics2D g2) {
